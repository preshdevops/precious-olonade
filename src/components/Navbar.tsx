...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:fixed focus:top-3 focus:left-3 focus:z-[100] focus:bg-[#4A8FE7] focus:text-[#0A0A0A] focus:px-4 focus:py-2 focus:rounded-lg focus:font-mono focus:text-xs font-bold shadow-2xl"
      >
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 glass backdrop-blur-xl border-b border-[rgba(255,255,255,0.07)] py-4 px-6 md:px-12 transition-all">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <a
            ...
            aria-label="Precious Olonade Home"
          >
            PO
          </a>
        ...
        {isOpen && (
          <div
            id="mobile-menu"
            className="md:hidden mt-4 pt-4 border-t border-[rgba(255,255,255,0.07)] flex flex-col gap-4 pb-2 animate-fadeIn"
          >
            {NAV_ITEMS.map((item) => {
              return (
                <a
